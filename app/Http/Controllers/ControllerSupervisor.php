...
use Carbon\Carbon;
use DB;
class ControllerSupervisor extends Controller
{
	...
	function __construct() {    
       ...
       // Session::put('user_login',"U01853"); //Aleja
        //Session::put('user_login',"U00001"); //CC
    ...
    **/
	public function index()
	{
		...
		return view('proyectos.supervisor.index',array(
			"fecha1" => $fechaActual,
            "fecha2" => $nuevafecha,
            "proyectos" => $proyecto,
        ...
        if (isset($request->all()['order']) && isset($ordena[0]['column']) && $ordena[0]['column'] != null && $ordena[0]['column'] != 0) {
            ...
            if ($colunma['orderable']) {
                ...
                $dir = $direcc[0]['dir'];
                     ...
                     if ($order == 'datou') { $order = 't.nombre'; } 
                else if ($order == 'datod') { $order = "s.propietario"; }
                else if ($order == 'datot') { $order = "CONCAT(CONVERT(varchar,l.fecha,103),' ',CONVERT(varchar,l.fecha,108))"; }
                else if ($order == 'datoc') { $order = "l.version_previa"; }
                else if ($order == 'datoci'){ $order = "l.version_posterior"; }
...
//echo $spt;   
    ...
    **/
    ...
    public function indexInspeccion($id_inspeccion,$abrir = "")
    {
        ...
        $inspec = DB::Table('ins_inspeccion as tbl1')
	    	    ...
	    	    ->leftJoin('gop_proyectos as pry1','pry1.id_proyecto','=','tbl1.id_tipo_proyecto')
                    ...
                    ->leftJoin('rh_personas as tbl3','tbl3.identificacion','=','tbl1.lider')
                    ...
                    ->select('tbl1.id_orden','tbl1.prefijo','tbl1.supervisor','tbl1.fecha_servidor','tbl1.lider','tbl1.aux1',
                        ...
                        'sede',
                        'tbl7.nombres as nombreC','tbl7.apellidos as apellidosC','tipo_inspeccion', 'coordenadas',
        ...
        ->select('ruta',DB::raw("concat((case when ins_foto.fecha is null or ins_foto.fecha < rutas_anexos.fecha_corte then 
        (select rutas_anexos.url_visualizacion from rutas_anexos where rutas_anexos.tipo_operacion = 'INSPANT') 
        else (select rutas_anexos.url_visualizacion from rutas_anexos where rutas_anexos.tipo_operacion = 'INSP') end), 
        ruta) as direccion"))
        ->join('rutas_anexos','rutas_anexos.tipo_operacion','=', DB::raw("'INSP'"))
        ->where('id_inspeccion','=',$id_inspeccion)
        ->where('id_tipo','=','T04')
        ->get();
        ...
        if($inspec->coordenadas) {
            list($latitud, $longitud) = explode(',', $inspec->coordenadas);
        }


        $sedes = DB::Table('gop_proyectos_sedes')->where('id',$inspec->sede)->get(['nombre_sede']);
        // dd($form);
    ...
    public function NuestrosValoresPdf($id_inspeccion, $abrir="")
    {
        if(!Session::has('user_login'))
        return Redirect::to('{{config("app.Campro")[2]}}/campro');
        ...
        $inspec = DB::Table('ins_inspeccion as tbl1')
                    ...
                    ->leftJoin('gop_proyectos as pry2','pry2.id_proyecto','=','tbl1.id_proyecto')
                    ->join('rh_personas as tbl2','tbl2.identificacion','=','tbl1.supervisor')
                    ->join('rh_cargos as tbl9','tbl9.id_cargo','=','tbl2.id_cargo')
                    
                    ->join('gop_ciudades_colombia as tblx', 'tblx.id_ciudad','=','pry2.id_ciudad')

                    ->leftJoin('rh_personas as tbl3','tbl3.identificacion','=','tbl1.lider')
                    ->join('rh_cargos as tbl10','tbl10.id_cargo','=','tbl3.id_cargo')
                    ->leftJoin('rh_personas as tbl4','tbl4.identificacion','=','tbl1.aux1')
                    ->leftJoin('rh_personas as tbl5','tbl5.identificacion','=','tbl1.aux2')
                    ->leftJoin('rh_personas as tbl6','tbl6.identificacion','=','tbl1.aux3')
                    ->leftJoin('rh_personas as tbl7','tbl7.identificacion','=','tbl1.conductor')
                    ...
                    ->select('tbl1.id_orden','tbl1.prefijo','tbl1.supervisor','tbl1.fecha_servidor','tbl1.lider','tbl1.aux1',
                        ...
                        'tbl4.nombres as nombreA1','tbl4.apellidos as apellidosA1',
                        'tbl5.nombres as nombreA2','tbl5.apellidos as apellidosA2',
                        'tbl6.nombres as nombreA3','tbl6.apellidos as apellidosA3', 'tblx.nombre as ciudad',
                        'sede','tbl2.id_cargo','tbl9.nombre','tbl2.nombres','tbl10.nombre as cargoL',
                        'tbl7.nombres as nombreC','tbl7.apellidos as apellidosC','tipo_inspeccion', 'coordenadas',
                        DB::raw(" IIF(tbl1.tipo_inspeccion=3,pry2.nombre,pry1.nombre) as proyecto "), 'pry2.subproceso as subproyecto',
                        'tbl8.nombre as estadoE','tbl1.calificacion','tbl1.charla_calificacion','tbl1.observacion')
                    ->get()[0];
        ...
        $firmaLider= DB::Table('rh_documentos_personas')
        ->select('adjunto',DB::raw("concat((case when rh_documentos_personas.fecha_adjunto is null or rh_documentos_personas.fecha_adjunto < rutas_anexos.fecha_corte then 
        (select rutas_anexos.url_visualizacion from rutas_anexos where rutas_anexos.tipo_operacion = 'ANTPUB') 
        else (select rutas_anexos.url_visualizacion from rutas_anexos where CONVERT(varchar,rh_documentos.id_proceso_rh) = rutas_anexos.tipo_operacion) end), 
        adjunto) as direccion"),'tbl2.id_persona')
        ->join('rh_documentos','rh_documentos.id_documento','=',DB::raw("'TD139'"))
        ->join('rh_personas as tbl2','tbl2.identificacion',"=",DB::raw("CONVERT(VARCHAR,'".$inspec->lider."')"))
        ->join('rutas_anexos','rutas_anexos.tipo_operacion','=', DB::raw('CONVERT(VARCHAR,rh_documentos.id_proceso_rh)'))
        ->where('rh_documentos_personas.id_persona',"=",DB::raw("CONVERT(VARCHAR,tbl2.id_persona)"))
        ->where('rh_documentos_personas.id_documento','=','TD139')
        ->get();

        $firmaSupervisor= DB::Table('rh_documentos_personas')
        ->select('adjunto',DB::raw("concat((case when rh_documentos_personas.fecha_adjunto is null or rh_documentos_personas.fecha_adjunto < rutas_anexos.fecha_corte then 
        (select rutas_anexos.url_visualizacion from rutas_anexos where rutas_anexos.tipo_operacion = 'ANTPUB') 
        else (select rutas_anexos.url_visualizacion from rutas_anexos where CONVERT(varchar,rh_documentos.id_proceso_rh) = rutas_anexos.tipo_operacion) end), 
        adjunto) as direccion"),'tbl2.id_persona')
        ->join('rh_documentos','rh_documentos.id_documento','=',DB::raw("'TD139'"))
        ->join('rh_personas as tbl2','tbl2.identificacion',"=",DB::raw("CONVERT(VARCHAR,'".$inspec->supervisor."')"))
        ->join('rutas_anexos','rutas_anexos.tipo_operacion','=', DB::raw('CONVERT(VARCHAR,rh_documentos.id_proceso_rh)'))
        ->where('rh_documentos_personas.id_persona',"=",DB::raw("CONVERT(VARCHAR,tbl2.id_persona)"))
        ->where('rh_documentos_personas.id_documento','=','TD139')
        ->get();
        ...
        $pdf = \PDF::loadView('proyectos/supervisor/ValoresPDF',
                array('insp' => $id_inspeccion,
                'fecha' => $fecha_actual,
                'hora' => $hora_actual,
                'latitud' => $latitud,
                ...
                'firmaLider' => $firmaLider,
                "firma" => $foto));
  
        return $pdf->stream('EOP-IN-01-RG-03 NUESTROS VALORES EN TERRENO[437].pdf');
    }

    public function inspeccionAmbientalPdf($id_inspeccion, $abrir="")
    {
        if(!Session::has('user_login'))
        return Redirect::to('{{config("app.Campro")[2]}}/campro');
        ...
        // dd($form);
        // dd($firmaSupervisor);
        $pdf = \PDF::loadView('proyectos/supervisor/InspeccionPDF',
                array('insp' => $id_inspeccion,
                'fecha' => $fecha_actual,
                'hora' => $hora_actual,
                'latitud' => $latitud,
    ...
    public function regenerarDetalleIPAL(Request $request) {
      ...
      if($id_inspeccion) {
        ...
        foreach ($detalle_ultimo_ipal_generado as $row) {
          $respuesta_item = 2;
          if($row->id_pregunta == 119) { $respuesta_item = 'Condiciones Optimas.'; }
 ...
 //Consulta permisos usuarios
    ...
    **/
    ...
    public function generaExcelAccionesCorrectivas(Request $request) {
        ...
        \Excel::create('ACCIONES_CORRECTIVAS__' . $this->fechaALong, function($excel) use($ipales) {            
                $excel->sheet('Ipal', function($sheet) use($ipales){
                    $titulos = [
                      ...
                      "ANALISIS DE CAUSAS",
                      "CAUSA RAIZ",
                      "TIPO DE ACCION A IMPLEMENTAR",
                      "DESCRIPCIÓN PLAN DE ACCION",
                      "FECHA DE ELABORACIÓN DE PLAN DE ACCION",
                      "PROCESO",
                      "RESPONSABLE",
                      "FECHA ESTABLECIDA PARA EJECUCION DEL PLAN DE ACCION",
                      "AVANCE DEL PLAN DE ACCION %",
                      "FECHA DE EJECUCION REAL DEL PLAN DE ACCION",
                      "EVIDENCIA",
                      "OBSERVACIONES",
                      "ESTADO"
                    ];
                    ...
                    for ($i=0; $i < count($ipales); $i++) {
                        ...
                        $consulta = "
                            select 
                              ...
                              (
                                  SELECT TOP(1) planAccionObsCierre.observacion_cierre
                                  FROM ins_plan_accion as planAccionObsCierre
                                  WHERE planAccionObsCierre.id_inspeccion = '" . $ipales[$i]->id_inspeccion . "'
                                      AND planAccionObsCierre.item = tbl2.id_pregunta
                              ) as observacionesCierre,

                              (
                                  SELECT TOP(1) planAccionEvidencia.evidencia
                                  FROM ins_plan_accion as planAccionEvidencia
                                  WHERE planAccionEvidencia.id_inspeccion = '" . $ipales[$i]->id_inspeccion . "'
                                      AND planAccionEvidencia.item = tbl2.id_pregunta
                              ) as evidencia
                             ...
                             FROM  [ins_inspeccion_detalle] as [tbl1] 
                              ...
                              INNER JOIN [gop_formularios_creacion] AS [tbl2] 
                                ...
                                AND [tbl2].[tipo_formulario] IN(1, 2, 3, 4, 5, 33, 34, 35, 43, 44, 46, 36, 37)
                                AND [tbl1].[version] = [tbl2].[version] 
                            
                              INNER JOIN [gop_formularios_creacion] AS tbl3 
                                ON tbl3.id_pregunta = [tbl2].[id_padre] 
                                AND [tbl3].[tipo_formulario] IN(1, 2, 3, 4, 5, 33, 34, 35, 43, 44, 46, 36, 37)
                        ...
                        if($ipalRes != null) {
                          foreach ($ipalRes as $key => $value) {
                              ...
                              $avance_plan_accion = 0;
                              if($ipales[$i]->fecha_cierre) {
                                $avance_plan_accion = 100;
                              }
                              ...
                              $sheet->row($fila +1, array(
                                  ...
                                  $value->item_num." ".$value->descrip_pregunta,      // "IDENTIFICACION DEL HALLAZGO"
                                  $value->planAccion,                                 // "CORRECION"
                                  "-",                                                // "ANALISIS DE CAUSAS"
                                  $value->causa,                                      // "CAUSA RAIZ"
                                  "CORRECTIVA",                                       // "TIPO DE ACCION A IMPLEMENTAR"
                                  "",                                                 // "DESCRIPCION PLAN ACCION"
                                  "",                                                 // "FECHA DE ELABORACION DE PLAN DE ACCION"
                                  $ipales[$i]->proyecto,                              // "PROCESO",
                                  "-",                                                // "RESPONSABLE" <-- COLOCAR CAMPO ADICIONAL EN APP MOVIL
                                  $ipales[$i]->fecha_objetivo_cierre,                 // "FECHA ESTABLECIDA PARA EJECUCION DEL PLAN DE ACCION",
                                  "{$avance_plan_accion} %",                          // "AVANCE DEL PLAN DE ACCION %"
                                  $ipales[$i]->fecha_cierre,                          // "FECHA DE EJECUCION REAL DEL PLAN DE ACCION",
                                  $existe_evidencia,                                  // "EVIDENCIA"
                                  utf8_encode($value->observacionesCierre),           // "OBSERVACIONES",
                                  $ipales[$i]->estado                                 // "ESTADO"
                              ));
        ...
        try {
	        ...
	        \Excel::create('IPALES ' . $this->fechaALong, function($excel) use($ipales) {            
	                $excel->sheet('Ipal', function($sheet) use($ipales){
	                    ...
	                    ,"EVALUACION GRABACION CHARLA OPERATIVA","ESTADO","DIRECCIÓN","LATITUD ORDEN","LONGITUD ORDEN","LATITUD INSPECCION", "LONGITUD INSPECCION", "EDAD COLABORADOR","EXPERIENCIA EN EL CARGO","PERIODO EN LA COMPAÑIA"];
	                    $sheet->fromArray($titulos);
	                    ...
	                    for ($i=0; $i < count($ipales); $i++) {
                          ...
                          if($ipales[$i]->coordenadas) {
                            list($latitud_inspeccion, $longitud_inspeccion) = explode(',', $ipales[$i]->coordenadas);
                            $latitud_inspeccion = str_replace(',', '.', $latitud_inspeccion);
                            $longitud_inspeccion = str_replace(',', '.', $longitud_inspeccion);
                          }
	                        ...
	                        if($ipalRes != null)
	                        {
	                            foreach ($ipalRes as $key => $value) {
	                                ...
	                                $sheet->row($fila +1, array(
	                                    ...
	                                    $ipales[$i]->actividad, // NOTA :: Antes $ipales[$i]->tipoOrden,
	                                    ...
	                                    $ipales[$i]->latitud,
	                                    $ipales[$i]->longitud,
                                      (string) $latitud_inspeccion,
                                      (string) $longitud_inspeccion,
	                                    $ipales[$i]->edad,
	                                    "",
	                                    ""
	                                    ));  
                ...
                if($pre == "rds")
                {
                    ...
                    if(isset( $request->all()['usuario']))
                    {
                        $fotos = DB::Table($pre . '_gop_fotos')
                                ...
                                ->select(DB::raw("('http://201.217.195.43/tprog/fotos/ordenes/' + ruta) as ruta"))
                                ->get();
                    }
                    else
                    {
                ...
                }else
                {
                  ...
                  if($pre == "apu") {
                    ...
                    $fotos = DB::Table($pre . '_gop_fotos')
                        ->where('id_orden',$orden)
                        ->whereBetween('fecha',[$request->all()['fecha1'],$request->all()['fecha2']])
                        ->select(DB::raw("('http://201.217.195.43/apu/tprog/fotos/ordenes/' + ruta) as ruta"))
                        ->get();
                  }
                }
	...
	return view('proyectos.supervisor.nuevoIpal',array(
     ...
     public function cargaNuevoIpal(Request $request){
        ...
        $tipoformulario=0;
        //Tipo de inspección
             if($dato==1){ $tipoformulario=1; }
        else if($dato==2){ $tipoformulario=2; }
        else if($dato==3){ $tipoformulario=20; }
        else if($dato==4){ $tipoformulario=26; }
	...
	$consulta = "	SELECT proyecto,prefijo_db,MAX(id_proyecto) AS id_proyecto
       ...
       //$dato=26;
        ...
        $canti=$canti/2;
             ...
             if($dato==1){ $canti = $canti + 3; }
        else if($dato==2){ $canti = $canti + 1; }
	...
	if(isset($request->all()['id_orden'])){
             ...
             if($datos['id_orden'] == null){$datos['id_orden']='';}
        }
    ...
    $sql_cargo_supervisor = "SELECT TOP 1
                              rh_cargos.nombre AS nombre_cargo,
                              rh_cargos.id_cargo AS id_cargo
                            FROM rh_personas
                              INNER JOIN rh_cargos
                                ON rh_personas.id_cargo = rh_cargos.id_cargo
                            WHERE identificacion IN('{$datos['supervisor']}');";
    $data_cargo_supervisor = DB::select($sql_cargo_supervisor);

    if(is_array($data_cargo_supervisor) && count($data_cargo_supervisor) > 0) {
      $id_cargo_supervisor      = $data_cargo_supervisor[0]['id_cargo'];
      $nombre_cargo_supervisor  = $data_cargo_supervisor[0]['nombre_cargo'];
    }
  }
        /*
	...
	if(isset($request->all()['tipo_inspeccion'])){
        ...
        if(in_array($id_cargo_supervisor, array('C1203'))) {
          $datos['tipo_ingreso_inspeccion'] = 'LN';
        }
        else {
          $datos['tipo_ingreso_inspeccion'] = 'SSL';
        }
