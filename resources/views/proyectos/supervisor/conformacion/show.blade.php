...
@section('content')
    ...
    <script type="text/javascript">
        ...
        var mesActual = {{$lider->mes}};
        var anioActual = {{$lider->anio}};
        ...
        function consultaEquipo(id)
        {
            var anio = document.querySelector("#txt_anio").value;
            var mes = document.querySelector("#txt_mes").value;
            ...
            consultaAjax("{{url('/')}}/transversal/supervision/wsWebConsulta", datos, 15000, "POST", 4);
        ...
        function consultaAjax(route,datos,tiempoEspera,type,opcion,collback,dato,ele)
        {
            ...
            $.ajax({
                ...
                success:function(data)
                {
                    ...
                    {
                        if(data.res != "")
                            window.location.href = "{{url('/')}}/transversal/supervision/conformacion/" + data.res;
                        else
                        {
                            ocultarSincronizacion();
                            mostrarModal(2,null,"Plan de supervisión","Usted no tiene plan de supervisión para la fecha seleccionada, espere un momento mientras se carga su última conformación.\n",0,"Aceptar","",null);
                            document.querySelector("#boton_1_modal").style.display = "none";
                            setTimeout(function()
                            {
                                window.location.href = "{{url('/')}}/transversal/supervision/conformacion/" + data.idMayor;
                            },4500);
                        }
