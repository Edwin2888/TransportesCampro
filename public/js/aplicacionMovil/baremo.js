...
function trabajarWeb()
{
  ...
  {
     consultaOrdenesBaremos(this.value);
  });
  ...
  consultaOrdenesBaremos(nodosID[0]);
  ...
  consultaOrdenesBaremos();*/
}
 var ArregloOrdenes = [];
 var nodosG = [];
function consultaOrdenesBaremos(nodo)
{ 
  ...
  var route = server + "consultaBaremoss";
  $.ajax({
    url: route,
    type: "POST",
    dataType: "json",
    ...
    success:function(data)
    {
      ...
      for (var i = 0; i < data.length; i++) {
        html += "<tr>";
          html += "<td>" + data[i].id_baremo + "</td>";
          html += "<td>" + data[i].actividad + "</td>";
          html += "<td><input type='number' value='" +  data[i].cant.replace(".00","") + "'></td>";
        html += "</tr>";
      }
...
function guardarBaremos()
{
  ...
  for (var i = 0; i < tbl.length; i++) {
    ...
    {
      alert("No puede guardar los baremos, ya que hay cantidades vacias");
      return;
    }
    arrayEnviar.push({
  ...
  $.ajax({
    ...
    success:function(data)
    {
      ...
      {
        var exis = 0;
        for (var i = 0; i < nodosG.length; i++) {
          if(nodosG[i] == dat[0].nodo)
          {
             exis = 1;
             break;
          }
...
{
  if(nodosG.length == document.querySelector("#nodos").children.length)
  {
     window.location.href = "http://201.217.195.43/camprolaravel/baremosAppMateriales";
  }
  else
  {
